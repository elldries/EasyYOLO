...
def create_dataset_structure(folder_name, base_path="DATA"):
    ...
    try:
        ...
        with open(readme_path, 'w', encoding='utf-8') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write("  1. Take screenshots during gameplay (use Windows Snipping Tool or similar)\n")
            f.write("  2. Images must show the targets you want to detect (enemies, heads, etc.)\n")
            f.write("  3. Recommended: 500-2000 images for good accuracy\n")
            f.write("  4. Image formats: .jpg, .png, .bmp\n")
            f.write("  5. Split: 70-80% in train/images, 20-30% in val/images\n\n")
            ...
            f.write("YOLO LABEL FORMAT:\n")
            f.write("=" * 80 + "\n")
            f.write("  Each line in .txt file represents ONE detection box:\n")
            f.write("  Format: <class_id> <x_center> <y_center> <width> <height>\n\n")
            f.write("  - class_id: Object type (0 = first class, 1 = second class, etc.)\n")
            f.write("  - x_center: Center X position (normalized 0.0 to 1.0)\n")
            f.write("  - y_center: Center Y position (normalized 0.0 to 1.0)\n")
            f.write("  - width: Box width (normalized 0.0 to 1.0)\n")
            f.write("  - height: Box height (normalized 0.0 to 1.0)\n\n")
            f.write("  Example (enemy head at center of 1920x1080 image):\n")
            f.write("  0 0.5 0.5 0.15 0.2\n\n")
            
            f.write("QUICK START GUIDE:\n")
            f.write("=" * 80 + "\n")
            f.write("  Step 1: Collect Screenshots\n")
            f.write("     - Play your game and take 500+ screenshots showing targets\n")
            f.write("     - Include different angles, distances, lighting conditions\n\n")
            f.write("  Step 2: Label Your Images\n")
            f.write("     - Download LabelImg or use online tool (Roboflow)\n")
            f.write("     - Draw boxes around targets (heads, bodies, enemies)\n")
            f.write("     - Save labels in YOLO format (.txt files)\n\n")
            f.write("  Step 3: Organize Files\n")
            f.write("     - Put 70-80% of images + labels in train/ folders\n")
            f.write("     - Put 20-30% of images + labels in val/ folders\n\n")
            f.write("  Step 4: Start Training\n")
            f.write("     - Run: python Train.py\n")
            f.write("     - Select this dataset folder\n")
            f.write("     - Choose model size (yolo11n.pt for fast, yolo11x.pt for best)\n")
            f.write("     - Train for 100+ epochs\n\n")
            ...
            f.write("=" * 80 + "\n")
            f.write("  Problem: Training fails\n")
            f.write("  Solution: Check that image and label file names match exactly\n\n")
            f.write("  Problem: Low accuracy\n")
            f.write("  Solution: Add more training images (aim for 1000+)\n\n")
            f.write("  Problem: Slow training\n")
            f.write("  Solution: Make sure GPU is enabled (Train.py will show Device: cuda)\n\n")
